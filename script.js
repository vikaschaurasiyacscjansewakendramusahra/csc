...
let services = DEFAULT_SERVICES.map(s => ({...s, portals:Array.isArray(s.portals)?s.portals.map(p=>[p[0],p[1]]):[], items:Array.isArray(s.items)?s.items.map(i=>[i[0],i[1]]):[]}));
let isAdmin = false;
function normalizeService(s){return {icon:String(s?.icon||'💼'),title:String(s?.title||'सेवा'),desc:String(s?.desc||''),portals:Array.isArray(s?.portals)?s.portals:[],items:Array.isArray(s?.items)?s.items.map(i=>Array.isArray(i)?[String(i[0]||''),String(i[1]||'')]:[String(i?.text||''),String(i?.url||'')]):[]};}
