...
function generateUPI(){
  ...
  try{
    if(typeof QRCode==='function'){
      new QRCode(box,{text:upiUrl,width:220,height:220,correctLevel:QRCode.CorrectLevel.M});
      if(error) error.textContent='';
    }else if(box){
      box.innerHTML='<a href="'+upiUrl.replace(/"/g,'&quot;')+'" style="display:inline-block;padding:18px;text-decoration:none;color:#805a0a;font-weight:900">📲 QR library load नहीं हुई। यहाँ टैप करके UPI App खोलें</a>';
      if(error) error.textContent='';
    }
  }catch(e){
    if(error) error.textContent='QR Generate नहीं हो पाया। नीचे UPI App बटन से भुगतान करें।';
  }
}
